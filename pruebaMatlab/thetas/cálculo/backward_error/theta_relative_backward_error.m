function [theta,M]=theta_relative_backward_error(f,nt,nd)
% [theta,M]==theta_relative_backward_error(f,nt,nd)
%
% Cálcula los valores theta de la función f mediante series de Taylor y
% errores relativos de tipo backward.
% 
% Datos de entrada:
...
fprintf('Theta values of relative backward errors for %s (nt=%d, nd=%d)\n',func2str(f),nt,nd);
%for k=1:length(M)
for k=7:7   
    ...
    eb = series_relative_back_taylor(f,m,nt);
    x=0;inc=1;
    for i=1:nd
