function pr_generalv1(test,f,dat,flag_figure)
% pr_general(test,f,dat,flag_figure)
%
...
% f{1}='expmbertay';f{2}='expmber';f{3}='expmtay';f{4}='exptaynsv3';f{5}='expm_newm';
% dat=[1 1 1 9 0];flag_figure=0;
% pr_general(test,f,dat,flag_figure)
...
%Versión v1(26-6-2019)
%Se ha modificado la última versión (pr_general), de manera que se utilizan
%los ficheros mat obtenidos a partir de la versión v1 de la generación de
%matrices

T=[];E=[];prods=[];cond=[];t=test{1};
for i=1:length(test)
    if i>1
        t=[t,'-',test{i}];
    end
    eval(test{i});
	[T1,E1,cond1,prods1]=calcula_errores(test{i},f,dat,nmat); 
	T=[T,T1];E=[E,E1];prods=[prods,prods1];cond=[cond,cond1];
end

make_graph(t,f,T,E,cond,prods,14,flag_figure)
end

function [T,E,cond,Np]=calcula_errores(test,f,dat,nmat)
fprintf('Test %s:\n',test); 
nf=length(f);T=zeros(nf,nmat);Np=zeros(nf,nmat);cond=zeros(1,nmat);
...
for i=1:nmat
    ...
    if isinf(condFA) || isnan(condFA)
        fprintf('\tLa matriz %d no formará parte de la gráfica normwise (condFA=%f)\n',i,condFA);
    end
    for j=1:nf
        try
            if dat(j)>=0
...
aux=[aux,nfun];
%aux='Figures\Fig_ratio_matrix_products';
