function pr_generalv4(test,f,datos,nmatMax,flag_figure)
% pr_generalv4(test,f,dat,nmatMax,flag_figure)
%
% Función que compara resultados entre diferentes tipos de métodos de 
% cálculo de funciones matriciales.
%
% Datos de entrada:
% - test:        Array de celdas con el nombre del directorio donde residen 
%                las matrices de test a emplear en la comparativa.
% - f:           Array de celdas con el nombre de los métodos de cálculo a 
%                comparar.
% - datos:       Array de celdas con el segundo parámetro de entrada de la 
%                función a invocar para cada uno de los métodos anteriores. 
%                Si la función no tiene tal argumento, estará vacío.
% - nmatMax:     Número máximo de matrices a considerar de cada tipo.
...
% datos{1}=1;datos{2}=1;datos{3}=9;datos{4}=0;flag_figure=0;
% pr_generalv4(test,f,datos,60,flag_figure)
...
% datos{1}=[3 4];datos{2}=1;datos{3}='';datos{4}=0;flag_figure=0;
% pr_generalv4(test,f,datos,flag_figure)

% Ejemplo de invocación para la acción exponencial y vector
% test{1}='expv_diag_hadamard_complex_n128_nd256';
% test{2}='expv_jordan_hadamard_complex_n128_boundvp10_maxmult5_nd256';
% test{3}='expv_toolbox_n128_nd256';
% test{4}='expv_eigtool_n128_nd256';
% f{1}='expmvtayn';f{2}='expAv_taylor';f{3}='expmvm';
% datos{1}=30;datos{2}=4;datos{3}='';nmatMax=60;flag_figure=0;
% pr_generalv4(test,f,datos,flag_figure)

% Versión v4 (16-12-2019)
% Parámetro datos en forma de array de celdas.
...
for i=1:length(test)
    ...
    eval(test{i});
    % Limitamos el número máximo de matrices a considerar
    if nmat>nmatMax
        nmat=nmatMax;
    end
	[T1,E1,cond1,prods1,M1,S1]=calcula_errores(test{i},f,datos,nmat); 
...
function [T,E,cond,Np,M,S]=calcula_errores(test,f,datos,nmat)
fprintf('Test %s:\n',test); 
...
for i=1:nmat
    ...
    for j=1:nf
        try
            if isempty(datos{j})==0 % Campo no vacío
                if exist('v','var')==1 % La variable v existe
                    tic
                    [C,m(j),s(j),np(j)]= feval(f{j},A,v,datos{j});
                    t(j)=toc;                    
                else
                    tic
                    [C,m(j),s(j),np(j)]= feval(f{j},A,datos{j});
                    t(j)=toc;
                end
            else
                if exist('v','var')==1 % La variable v existe 
                    tic
                    [C,m(j),s(j),np(j)]= feval(f{j},A,v);
                    t(j)=toc;                    
                else
                    tic
...
figure(5)
sim{1}='+k'; sim{2}='xk'; sim{3}='sk'; sim{4}='vk'; sim{5}='*k'; sim{6}='ok'; sim{7}='dk'; sim{8}='pk'; sim{9}='hk';
t=['plot(1:np,M(',int2str(1),',:),''',sim{1},''''];
for i=2:nmet
    t=[t,',1:np,M(',int2str(i),',:),''',sim{i},''''];
end
t=[t,')'];
eval(t);
...
xlabel('Matrix')
ylabel('Approximation polynomial order')

aux=['Figures\polynomial_order_',test];
aux=[aux,nfun];
