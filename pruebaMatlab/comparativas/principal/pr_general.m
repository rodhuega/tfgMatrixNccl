function pr_general(test,f,dat,flag_figure)
...
for i=1:nmat
    ...
    for j=1:nf
        try
            ...
            %fprintf('\tMatriz %d, %s, m=%d, s=%d, Er=%.2e\n',i,f{j},m(j),s(j),Er(j));
            if Er(j)>=10 || isnan(Er(j)) % Si el error relativo es demasiado grande, 
                         % prescindimos de la matriz
                flag_error=1;
                fprintf('\tMatriz %d no considerada. %s: error relativo=%.3e\n',i, f{j},Er(j));
            end
        catch
            flag_error=1;
            fprintf('\tMatriz %d no considerada. Ha habido un error al ejecutar %s\n',i,f{j})
        end
    end
    if flag_error
        continue
