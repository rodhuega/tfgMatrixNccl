function [m,s,pB,nProd]=selecciona_ms_cosh_hermite_sinEstNorma(plataforma,A)
...
% matriciales, para aplicar la función coseno hiperbólico sobre A mediante 
% Hermite.
%
% Datos de entrada:
...
beta_min = beta_cosh_hermite_sinEstNorma(b,d,2,q);
if beta_min<=Theta(2)
    m=2; % q=2
    return
end
...
% m = 4
beta_min = min(beta_min,beta_cosh_hermite_sinEstNorma(b,d,4,q));
if beta_min<=Theta(3)
    m=4; % q=2
    return 
end
...
d(6)=min(d(3)^2,d(2)^3);
beta_min = min(beta_min,beta_cosh_hermite_sinEstNorma(b,d,6,q));
if beta_min<=Theta(4)
    m=6; % q=3
    return
end
...
% m = 9
beta_min9 = min(beta_min,beta_cosh_hermite_sinEstNorma(b,d,9,q));
...
% m = 12
beta_min12 = min(beta_min9,beta_cosh_hermite_sinEstNorma(b,d,12,q));
if beta_min12<=Theta(6)
    m=12; 
    % AÑADIDO. FALTA LA POTENCIA CUARTA Y QUE q SEA IGUAL A 4, SI m VALE 12
    % ESTO NO SERÁ NECESARIO SI q PUEDE VALER 3 PARA m=12
    q=4;
    switch plataforma
        case 'sinGPUs'
            pB{4}=pB{3}*pB{1};
...
d(6)=min(d(6),d(4)*d(2));
beta_min12 = min(beta_min12,beta_cosh_hermite_sinEstNorma(b,d,12,q)); %We have new information with pB{4}=B^4
if beta_min12<=Theta(6)
    m=12; % q=4
    s=0;
    return
...
s12 = ceil(log2(beta_min12/Theta(6))/2);
beta_min16 = min(beta_min12,beta_cosh_hermite_sinEstNorma(b,d,16,q));
...
error('selecciona_ms_cosh_hermite_sinEstNorma:NoParam','Cannot find valid parameters, check matrix')
end