function [m,s,pB,nProd]=selecciona_ms_cos_taylor_sinEstNorma(plataforma,A)
...
% m = 9
beta_min9 = min(beta_min,beta_cos_taylor_sinEstNorma(b,d,9,q));
...
% m = 12
beta_min12 = min(beta_min9,beta_cos_taylor_sinEstNorma(b,d,12,q));
...
d(6)=min(d(6),d(4)*d(2));
beta_min12 = min(beta_min12,beta_cos_taylor_sinEstNorma(b,d,12,q)); %We have new information with pB{4}=B^4
if beta_min12<=Theta(6)
    m=12; % q=4
    s=0;
    return
...
s12 = ceil(log2(beta_min12/Theta(6))/2);
beta_min16 = min(beta_min12,beta_cos_taylor_sinEstNorma(b,d,16,q));
...
error('selecciona_ms_cos_taylor_sinEstNorma','Cannot find valid parameters, check matrix')
end
