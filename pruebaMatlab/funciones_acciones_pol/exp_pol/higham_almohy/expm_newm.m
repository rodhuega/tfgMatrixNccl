function [F,m,s,np] = expm_newm(A,schur_fact)
%Modificado para calcular el número de productos
...
t = eval_alpha(A,1);
    ...
    if eta1 <= theta(1) && t == 0
        m=m_vals(1);
        [F,npaux] = PadeApproximantOfDegree(m_vals(1));
        np=np+npaux;
        schur_backtrans
        return
    end

A4 = A2*A2; have_A4 = 1;np=np+1;
eta2 = max( norm(A4,1)^(1/4), normAm(A2,3)^(1/6) );
t = eval_alpha(A,2);

    if eta2 <= theta(2) && t == 0
        m=m_vals(2);
        [F,npaux] = PadeApproximantOfDegree(m_vals(2));
        np=np+npaux;
        schur_backtrans
        return
    end

A6 = A2*A4; have_A6 = 1;np=np+1;
...
for i = 3:4
    if eta3 <= theta(i) && h(i) == 0
        m=m_vals(i);
        [F,npaux] = PadeApproximantOfDegree(m_vals(i));
        np=np+npaux;
        schur_backtrans
        return
    end
...
m=m_vals(end);
[F,npaux] = PadeApproximantOfDegree(m_vals(end));
np=np+npaux;
...
%%%%Nested Functions%%%%
    ...
    function [F,np] = PadeApproximantOfDegree(m)
    ...
    function schur_backtrans
        if schur_fact, F = Q*F*Q';np=np+16; end
    end
