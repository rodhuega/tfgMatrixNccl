function gen_matrices_jordan_random_real(F,n,bound_vp,max_mult,nmat,nd)
% gen_matrices_jordan_random_real(F,n,bound_vp,max_mult,nmat,nd)
%
% Genera matrices de test de Jordan a partir de matrices V aleatorias con 
...
% gen_matrices_jordan_random_real(@exp,128,10,5,100,256)

digits(nd);
t0=sprintf('%s_jordan_random_real_n%d_boundvp%d_maxmult%d_nd%d',func2str(F),n,bound_vp,max_mult,nd);
try
    warning off
...
for k=1:nmat
    tic
    [A,FA,condV]=jordan_random_real(F,n,bound_vp,max_mult,nd);
    normA=norm(A,1);
    condFA=funm_condest1(A,F);
    if max(max((isnan(A))))||max(max((isinf(A))))
        flag_error=-2;
        fprintf('La función sobre la matriz %d ha dado error (inf o nan)\n',k);
    elseif max(max((isnan(FA))))||max(max((isinf(FA))))
        flag_error=-3;
        fprintf('No se ha podigo generar la matriz %d, pues ha dado error (inf o nan)\n',k);
    else
        flag_error=0;
    end
