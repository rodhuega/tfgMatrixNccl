function gen_matrices_toolbox_eigtool(F,n,nd)
...
% - condV:  Número de condición de la anterior matriz V (vectores propios).
...
for k=1:lm
    ...
    [FA,normA,condFA,e]=almacena_matrices(F,FT,im,A,nd);
    if e==0
        j=j+1;
        t=sprintf('save %s/%s_%d.mat A FA normA condFA',t0,t0,j);
        eval(t);
    elseif e==14
        ...
        fprintf('Bad matrix %d: no ha convergido  Taylor para m=%d.\n',k,M(e));
        fprintf(fileID,'Bad matrix %d: dos aproximaciones consecutivas de Taylor siempre han dado error.\n',k); 
    end
...
fileID = fopen(t,'w');
fprintf(fileID,'nmat=%d;',j);
fclose(fileID);
end

function [FA,normA,condFA,e]=almacena_matrices(F,FT,im,A,nd)
digits(nd);
A=vpa(A);
normA=[];
condFA=[];
...
fin=0;
while e==0&&fin==0&&im<14 %im=14 corresponde a m=64 
    im=im+1;
    if mod(im,2)==1
...
function [B,e,s,er]=taylor_aux_exp(pA,im,nd)
digits(nd);
n=size(pA{1},1);
er=0;
M= [2 4 6 9 12 16 20 25 30 36 42 49 56 64];
...
function [B,e,s,er]=taylor_aux_cos(pA,im,nd)
digits(nd);
n=size(pA{1},1);
er=0;
M= [2 4 6 9 12 16 20 25 30 36 42 49 56 64];
