function genera_matrices_diag_hadamard_real(F,n,maxv,nmat,nd)
% genera_matrices_diag_hadamard_real(F,n,maxv,nmat,nd)
%
% Genera matrices de test diagonalizables con transformaciones ortogonales
% a partir de matrices V de Hadamard, con valores propios reales de 
% magnitud creciente. Por cada matriz se genera un fichero .mat que 
% almacena:
...
% genera_matrices_diag_hadamard_real(@exp,128,5,100,256)

digits(nd);
t0=sprintf('%s_diag_hadamard_real_n%d_maxv%d_nd%d',func2str(F),n,maxv,nd);
try
    warning off
...
for i=1:nmat
    tic
    D=rand(n,1)-0.5;
    V=vpa(hadamard(n)/vpa(sqrt(vpa(n))));
    Vd=double(V);
    condV=cond(double(V));
    nA=vpa(norm(Vd*diag(D)*Vd',1));
    D=vpa(vpa(D)*i*maxv/nA);
    A=double(vpa(V*diag(D)*V'));
    normA=norm(A,1);
    condFA=funm_condest1(A,F);
    fD=vpa(F(D));
	FA=double(vpa(V*diag(fD)*V'));
