...
def get_identifier():
    return "CPIStallTexThrottle"

def get_name():
    return "CPI Stall 'TEX Throttle'"

def get_description():
    return "Warp stall analysis for 'TEX Throttle' issues"
...
def apply(handle):
    ...
    if ccMajor == 7 and ccMinor >= 0:
        ...
        warpCyclesPerStall = action.metric_by_name("smsp__average_warps_issue_stalled_tex_throttle_per_issue_active.ratio").as_double()
        ...
        warpCyclesPerStall = action.metric_by_name("smsp__warp_cycles_per_issue_stall_tex_throttle").as_double()
    ...
    if issueActive < 0.8 and 0.3 < (warpCyclesPerStall / warpCyclesPerIssue):
        message = "On average each warp of this kernel spends {:.1f} cycles being stalled waiting for the L1TEX instruction queue to be not full. This represents about {:.1f}% of the total average of {:.1f} cycles between issuing two instructions. This stall reason is high in cases of extreme utilization of the L1TEX pipeline. If applicable, consider combining multiple lower-width memory operations into fewer wider memory operations and try interleaving memory operations and math instructions.".format(warpCyclesPerStall, 100.*warpCyclesPerStall/warpCyclesPerIssue, warpCyclesPerIssue)
