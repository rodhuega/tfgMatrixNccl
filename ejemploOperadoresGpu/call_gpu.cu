...
void funcion_matricial::power( ) {
  pA.push_back( pA[0] * pA[pA.size()-1] );
  nProd++;
}
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) {
  ...
  if( !strcmp( comando, "init" ) ) {
    if( initiated ) return;
    if( nrhs!=4 ) {
      mexErrMsgIdAndTxt("MATLAB:call_gpu:invalidNumInputs","Evaluation method as second argument and a matrix as a third argument both required.");
    } 
    char funcion[80];
